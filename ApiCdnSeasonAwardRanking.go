...
/*
//	以下 CdnGiftRanking とメンバーは同一（2024-10-04時点では存在しないメンバーがある）
//	当面 CdnGiftRankingを使う
type CdnSeasonAwardRanking struct {
	TotalScore  int         `json:"total_score"` //	2024-10-04時点では存在しない
...
type SarRanking struct {
	RoomID  int    `json:"room_id"`
	Rank    int    `json:"rank"`
	Score   int    `json:"score"` //	2024-10-04時点では存在しない
...
func ApiCdnSeasonAwardRanking(
	...
	turl := fmt.Sprintf("https://public-api.showroom-cdn.com/season_award_ranking/%d", gift_id)
